...
class DataFetchCubit extends BaseCubit<DataFetchState, List<GitRepoEntity>> {
  ...
  //  Contador para manejar la paginación de los resultados de la API
  int _page = 1;
  ...
  Future<void> fetchData() async {
    ...
    if (!isConnected) {
      await getGitHubReposItemsFromDatabase(); // Fetch from database si no hay internet
    } else {
      await getGitHubReposItemsFromApi(); // Fetch from API si internet es available
    }
  }
  ...
  DateTime? _lastRefreshTime; // Mantener timestamp of last refresh

  //  Refrescar los datos si han pasado al menos 30 minutos desde
  //  la última actualización, gestionando el tiempo con _lastRefreshTime.
  Future<void> refreshData() async {
    if (_lastRefreshTime == null ||
        DateTime.now().difference(_lastRefreshTime!) >= const Duration(minutes: 30)) {
      try {
        _lastRefreshTime = DateTime.now(); // Update ultimo refresh timestamp
      } catch (e) {
      }
    }else{
